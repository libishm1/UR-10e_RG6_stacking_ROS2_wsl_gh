"""Verify plan+execute on all three planning groups now that the gripper
controller is a JointTrajectoryController.

Safe speeds throughout.
"""
import rclpy
...
def plan_exec(node, mg, group, joint_targets, label):
    goal = MoveGroup.Goal()
...
def main():
    ...
    away = dict(zip(UR_JOINTS, [0.5, -1.3, 1.0, -1.4, 0.2, 0.0]))

    print("Group 1: ur_manipulator → away pose (arm only)")
    a = plan_exec(n, mg, "ur_manipulator", away, "ur_manipulator")

    print("Group 2: rg6_gripper → close (rg6_joint=0.0)")
    b = plan_exec(n, mg, "rg6_gripper", {"rg6_joint": 0.0}, "rg6_gripper")

    print("Group 3: arm_with_gripper → home + open gripper")
    combined = dict(home); combined["rg6_joint"] = 1.0
    c = plan_exec(n, mg, "arm_with_gripper", combined, "arm_with_gripper")
