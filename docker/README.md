...
Builds and runs this workspace inside a self-contained ROS 2 Humble container.
...
|-------------|-----------------------------------------------------------------|--------------------------------------------------------------------------------------|
| rviz_view   | RViz with joint sliders                                         | `docker compose -f docker/docker-compose.yml up rviz_view`                           |
| ur_fake     | Full UR driver + controllers, simulated hardware                | `docker compose -f docker/docker-compose.yml up ur_fake`                             |
| ur_real     | Full UR driver against a physical UR10e (set `ROBOT_IP`)        | `ROBOT_IP=192.168.1.102 docker compose -f docker/docker-compose.yml up ur_real`      |
| rosbridge   | Exposes ROS 2 on `ws://localhost:9090` for Grasshopper/roslibpy | `docker compose -f docker/docker-compose.yml up rosbridge`                           |
| shell       | Interactive bash inside the container                           | `docker compose -f docker/docker-compose.yml run --rm shell`                         |
...
├── src/                    ← bind-mounted into /workspace/src
├── install/                ← regenerated by entrypoint colcon build
└── ...
```
